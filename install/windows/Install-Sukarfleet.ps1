...
param(
  ...
  # --- enrollment ---
  # Run from a console-generated one-click installer. Everything this script would otherwise ask
  # for -- machine name, mesh address, node port, network name, the peer to dial, the network
  # secret -- comes from a base64 payload in $env:SUKARFLEET_ENROLL_PAYLOAD, and the install ends
  # by pairing with the machine that generated it. Nobody types anything.
  #
  # The payload is NOT a parameter, on purpose: a command line is readable by every process on the
  # box, and this one carries the fleet's mesh secret.
  [switch] $Enroll,
...
# The decoded enrollment payload, or $null when this is an ordinary install. StrictMode 2.0 throws
# on a read of a variable that was never assigned, so it is declared here rather than at first use.
$script:EnrollPayload = $null
...
# A Windows account name is not an SSH account name. src/pairing.ts validates every bundle's
# sshUser against /^[a-z_][a-z0-9_-]{0,31}$/ -- the names sshd will accept, and never one that could
# start with '-' and be read as an option by `ssh -l` -- and it applies that rule to the LOCAL
# bundle too. So a machine whose Windows user is 'Ariel' fails its own validation and pairing dies
# with "This machine has no usable SSH identity yet", which names the symptom and not the cause.
#
# The value is inert on this platform anyway: the admin lane is off here and no peer dials a Windows
# node's lane, which is the same reasoning that lets a Windows node advertise a host key for an sshd
# it does not run. So it is folded to a name the validator accepts rather than left to break pairing
# on any machine whose owner capitalised their own name.
...
# Decodes $env:SUKARFLEET_ENROLL_PAYLOAD and becomes every argument this run was not given. Called
# once, immediately before the dispatch, and never in the elevated child: the child is handed a
# secret FILE and a normal argument list, exactly as it is on an ordinary install, so nothing about
# elevation has to learn what an enrollment is.
function Read-EnrollPayload {
  $raw = $env:SUKARFLEET_ENROLL_PAYLOAD
  if (-not $raw) {
    Write-Die '-Enroll was passed but SUKARFLEET_ENROLL_PAYLOAD is empty. Run the generated .cmd rather than this script directly.'
  }
  ...
  try {
    ...
    Write-Die "the enrollment payload could not be decoded: $($_.Exception.Message)"
  }

  # Every field is read through Get-Prop and then checked, because StrictMode 2.0 turns a missing
  # property into a crash whose message names PowerShell rather than the file that is actually
  # wrong. A truncated payload should say it is truncated.
  if ((Get-Prop -Object $p -Name 'v' -Default 0) -ne 1) {
    Write-Die 'that installer was generated by a different version of sukarfleet than this one understands.'
  }
  foreach ($field in @('machine', 'meshIp', 'networkName', 'meshSecret', 'enrollId', 'token', 'pairHost')) {
    if (-not (Get-Prop -Object $p -Name $field)) { Write-Die "the enrollment payload has no '$field'." }
  }
  $seeds = @(Get-Prop -Object $p -Name 'seeds' -Default @())
  if ($seeds.Count -eq 0) {
    Write-Die 'the enrollment payload names no mesh peer to dial, so this machine would have no way into the network.'
  }

  $expiresMs = [double] (Get-Prop -Object $p -Name 'expiresMs' -Default 0)
  if ($expiresMs -gt 0) {
    $nowMs = [double] ([DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
    if ($nowMs -gt $expiresMs) {
      $when = [DateTimeOffset]::FromUnixTimeMilliseconds([long] $expiresMs).UtcDateTime.ToString('u')
      # Refused here rather than after installing half a fleet node: the pairing at the end would
      # fail anyway, and a machine with a config, a service and no fleet is worse than no machine.
      Write-Die "this installer expired at $when. Generate a new one from the console that made it; nothing was installed."
    }
  }

  $script:MachineName = [string] $p.machine
  $script:MeshIp      = [string] $p.meshIp
  $script:NetworkName = [string] $p.networkName
  $script:PeerUri     = @($seeds | ForEach-Object { [string] $_ })
  $role = [string] (Get-Prop -Object $p -Name 'role' -Default 'roamer')
  if ($role -eq 'anchor' -or $role -eq 'roamer') { $script:Role = $role }
  $np = [int] (Get-Prop -Object $p -Name 'nodePort' -Default 0)
  if ($np -gt 0) { $script:NodePort = $np }
  $lp = [int] (Get-Prop -Object $p -Name 'listenPort' -Default 0)
  if ($lp -gt 0) { $script:ListenPort = $lp }
  $rb = [string] (Get-Prop -Object $p -Name 'releaseBase' -Default '')
  if ($rb -and -not $TrayReleaseBase) { $script:TrayReleaseBase = $rb }

  # The secret goes to the same staged file an interactive run would have prompted into, so the
  # elevated hand-off, the ownership question and the shred are all one code path with one set of
  # rules rather than two that can disagree.
  New-PrivateDir -Path $StateDir -Label 'state dir'
  Write-PrivateFile -Path $PendingSecret -Content ([string] $p.meshSecret + "`n")
  $script:MeshSecretFile = $PendingSecret

  $script:EnrollPayload = $p
  Write-Step "enrolling as '$script:MachineName' at $script:MeshIp into fleet '$script:NetworkName'"
  Write-Note "  peer to dial: $($script:PeerUri -join ', ')"
  Write-Note '  the mesh secret was written to a file only you can read; the elevated stage consumes and overwrites it.'
}

# The last step of an enrolled install: spend the token. This is the ordinary console pairing call,
# made by a script instead of by a person -- /api/ui/pair/redeem, on this machine's own daemon, over
# loopback, which is the only place that route answers at all.
function Invoke-Enrollment {
  param([Parameter(Mandatory)] [int] $Port)

  $p = $script:EnrollPayload
  $pairHost = [string] $p.pairHost
  $pairPort = [int] (Get-Prop -Object $p -Name 'pairPort' -Default $Port)
  $body = ConvertTo-Json -Compress -InputObject ([ordered]@{
    enrollId = [string] $p.enrollId
    token    = [string] $p.token
    host     = $pairHost
    port     = $pairPort
  })

  Write-Step "pairing with $pairHost`:$pairPort using the enrollment token"

  # The mesh service is up by now, but "the service is running" and "the overlay has a route to the
  # other machine" are not the same moment -- the same lesson install-elevated.sh learned about the
  # TUN address. So an unreachable answer is retried; a refused one is not, because a token the
  # other machine has already rejected will be rejected again in five seconds.
  for ($attempt = 1; $attempt -le 6; $attempt++) {
    $result = $null
    try {
      $res = Invoke-WebRequest -Uri "http://127.0.0.1:$Port/api/ui/pair/redeem" -Method POST `
        -ContentType 'application/json' -Body $body -UseBasicParsing -TimeoutSec 30
      $result = $res.Content | ConvertFrom-Json
    } catch {
      Write-Note "  attempt $attempt/6: the local node did not answer the pairing call ($($_.Exception.Message))"
    }

    if ($result) {
      if (Get-Prop -Object $result -Name 'ok' -Default $false) {
        Write-Step "paired with $(Get-Prop -Object $result -Name 'peer' -Default $pairHost). This machine is in the fleet."
        return $true
      }
      $reason  = [string] (Get-Prop -Object $result -Name 'reason' -Default '')
      $message = [string] (Get-Prop -Object $result -Name 'message' -Default 'the pairing was refused')
      if ($reason -ne 'unreachable') {
        Write-Warn "pairing was refused: $message"
        return $false
      }
      Write-Note "  attempt $attempt/6: $message"
    }
    if ($attempt -lt 6) { Start-Sleep -Seconds 5 }
  }

  Write-Warn "could not reach $pairHost`:$pairPort over the mesh after six tries. Everything else is installed: open the console and use the Pair screen, or re-run the generated .cmd while the token is still valid."
  return $false
}
...
function Invoke-ElevatedStage {
  ...
  # The node's own port, scoped to the mesh subnet rather than to a program. Peers reach this
  # machine at <meshIp>:<nodePort> for gossip, git-over-HTTP sync and /pair/hello, and Windows
  # Firewall filters the TUN adapter like any other. Without this rule the machine joins the mesh,
  # answers nothing, and looks like a network problem -- which nobody is watching for on a
  # one-click install.
  #
  # Scoped by RemoteAddress, not by -Program: the node runs under whatever bun.exe the user stage
  # installed, and a rule pinned to that path stops working the first time Bun is upgraded.
  if ($MeshIp) {
    $octets = $MeshIp.Split('.')
    if ($octets.Count -eq 4) {
      $meshSubnet = "$($octets[0]).$($octets[1]).$($octets[2]).0/24"
      $nodeRule = "sukarfleet node (TCP $NodePort from $meshSubnet)"
      if (-not (Get-NetFirewallRule -DisplayName $nodeRule -ErrorAction SilentlyContinue)) {
        try {
          [void] (New-NetFirewallRule -DisplayName $nodeRule -Direction Inbound -Action Allow `
            -Protocol TCP -LocalPort $NodePort -RemoteAddress $meshSubnet -Profile Any)
          Write-Step "firewall: allowed inbound TCP/$NodePort from $meshSubnet"
        } catch { Write-Warn "could not add the node firewall rule: $($_.Exception.Message)" }
      } else { Write-Note "firewall rule already present: $nodeRule" }
    }
  } else {
    Write-Warn "no mesh address is known yet, so the node's own port was not opened. Peers will reach the mesh and then find nothing answering on $NodePort. Re-run once the address is set."
  }

  # --- the clock ------------------------------------------------------------
  # The daemon vets its clock before committing, and a machine whose time service is stopped
  # reports 'unknown', which HOLDS auto-commit: the node joins, gossips, and never commits a thing.
  # Stock Windows leaves W32Time set to start on demand and it is frequently not running at all.
  # Nudged here rather than left for someone to notice, because on a one-click install nobody is
  # reading the fault list. A failure is a warning, never fatal: an unvetted clock costs commits,
  # not the install.
  try {
    $w32 = Get-Service -Name 'w32time' -ErrorAction Stop
    if ($w32.StartType -eq 'Disabled') { Set-Service -Name 'w32time' -StartupType Automatic }
    if ($w32.Status -ne 'Running') { Start-Service -Name 'w32time' }
    [void] (Invoke-Native -Exe 'w32tm.exe' -Arguments @('/resync'))
    Write-Step 'time service is running; asked it to resync so the daemon will vet the clock'
  } catch {
    Write-Warn "could not start the Windows time service: $($_.Exception.Message). The node will report its clock as unvetted and hold auto-commit until the time service runs."
  }
...
function Invoke-UserStage {
  ...
  # --- 7b. enrollment -------------------------------------------------------
  # After /health and before the banner: the banner tells the operator what state the machine is in,
  # and "paired" or "not paired yet" is the most important line in it on an enrolled install.
  $enrolled = $null
  if ($script:EnrollPayload) {
    if ($healthy) {
      $enrolled = Invoke-Enrollment -Port $port
    } else {
      $enrolled = $false
      Write-Warn 'skipping the pairing step: the node is not answering, so there is nothing to pair with yet.'
    }
  }
...
# Before anything is dispatched, and never in the elevated child: by the time that child runs, the
# payload has already become an ordinary argument list and a staged secret file.
if ($Enroll -and $Stage -ne 'Elevated') { Read-EnrollPayload }
