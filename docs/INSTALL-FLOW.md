...
## 7b. The other door: an installer generated by the console

Everything above is the journey for a person at a keyboard on a machine they are installing. There
is a second door, added after this document was first written, and it inverts the whole flow: on
Windows, a machine already in the fleet generates a `.cmd` that carries the network secret, an
allocated mesh address, the peer to dial and a single-use enrollment token, and the new machine is
double-clicked into the fleet with nothing typed on it.

It does not replace this journey, and it cannot. Something has to be the first machine, and a fleet
of one has nothing to generate an installer from. What it replaces is sections 2.4 and 2.6 — the
identity card and the pairing screen — for every machine after the first.

Details and the security posture: [`install/windows/README.md`](../install/windows/README.md) and
[`SECURITY.md`](../SECURITY.md#trust-model). The equivalent for Linux does not exist yet.
