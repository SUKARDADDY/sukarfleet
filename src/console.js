...
//      `api_call` command, never fetch(): the webview cannot reach the daemon directly (no
//      CORS there), and the Rust side is what actually knows which machine it is talking to.
//   2. Server data reaches the DOM only through textContent. No innerHTML anywhere.
//   3. There is no window.confirm/alert/prompt in this webview. Every destructive action
//      swaps its trigger button for an inline confirm strip instead.

// ── copy ──────────────────────────────────────────────────────────────────

const FORWARD = (m) => `ssh -N -L 7711:127.0.0.1:7710 ${m}`;
const REMOTE_CONSOLE = (m) => `Forward it with \`${FORWARD(m)}\` and open http://127.0.0.1:7711/ui/.`;

const COPY = {
  connecting: 'Connecting…',
  live: 'Live',
  offline: 'Cannot reach this daemon. The page keeps retrying.',
  reconnecting: 'Restarting. Reconnecting…',
  ...
  runConfirmPrompt: (m, why) => `This looks destructive (${why}). It will run as root on ${m}. Typing the name below is friction, not a safety check — the target daemon is what actually enforces the refusal rules.`,
...
// Ported from src/mcp.ts ADMIN_REFUSAL_ADVICE (lines 91-131). This supersedes app.js's own
// COPY.refusal: same wording the MCP tool gives an agent, plus 'agent-origin-refused', which
// app.js's copy lacks entirely.
...
// An inline confirm strip: the only confirmation mechanism this page has, because the webview
// has no window.confirm/alert/prompt. `requireText` (optional {label, id}) adds a "type this to
// confirm" input, used only by the admin destructive-command tripwire.
function confirmStrip(opts) {
  const wrap = el('div', 'confirm-strip');
  wrap.appendChild(el('p', null, opts.message));
  let textInput = null;
  if (opts.requireText) {
    const field = el('p', 'field');
    const label = el('label', null, opts.requireText.label);
    const inputId = opts.requireText.id || 'confirm-strip-input';
  ...
  const confirmBtn = el('button', opts.danger ? 'danger' : 'primary', opts.confirmLabel || 'Confirm');
  confirmBtn.type = 'button';
  const cancelBtn = el('button', 'quiet', opts.cancelLabel || 'Cancel');
  cancelBtn.type = 'button';
  cancelBtn.addEventListener('click', () => { if (opts.onCancel) opts.onCancel(); });
  confirmBtn.addEventListener('click', () => {
    if (opts.requireText && !textInput.value.trim()) { textInput.focus(); return; }
    opts.onConfirm(textInput);
  });
...
// ── transport ─────────────────────────────────────────────────────────────
// The webview cannot reach the daemon (no CORS there) -- every request crosses into Rust via
// invoke("api_call", ...), which does the actual HTTP call against loopback and hands back
// { status, body }. status 0 means the Rust side could not reach the daemon at all.
...
async function api(path, opts) {
  ...
  if (!tauriInvoke) throw new ApiError(0, COPY.networkFailed);
  let res;
  try {
    res = await tauriInvoke('api_call', { method, path, body: o.body });
  } catch {
...
const store = {
  ...
  pollAttempted: false,
  restarting: false,
  // Session-only latch: dismissing the setup takeover never persists past this page load.
  ...
  // Pending inline confirms. Lifted into store (rather than local closures) because every
  // screen that shows one is rebuilt from scratch on every 3s poll -- state driven from here
...
// at it. It is not a trap: an explicit hash always wins.
...
function renderHeader() {
  ...
  document.title = self ? `${machine} · sukarfleet` : 'sukarfleet';
  ...
  setText('conn-text', store.online ? COPY.live : store.restarting ? COPY.reconnecting : COPY.offline);

  show($('setup-banner'), setupIncomplete() && activeScreen() !== 'setup');

  const offline = store.pollAttempted && !store.online;
...
function renderSetupCards() {
  ...
  // Admin-lane pairing, NOT mesh membership -- adminStatus[].paired requires pinned host keys,
  // which is the only honest signal that the pairing step is actually done.
  ...
  renderRestartControl($('identity-restart-actions'), 'identity-restart-result', 'identityRestart', 'quiet');
...
// Driven off store.confirm.revoke rather than a local click handler, because the whole peers
// table is torn down and rebuilt on every 3s poll -- a pending confirm has to be re-derived
// from state on each rebuild, or a poll landing mid-confirm would silently discard it.
function revokeCell(machine) {
  if (store.confirm.revoke === machine) {
    return confirmStrip({
      message: COPY.revokeConfirm(machine, selfMachine()),
      confirmLabel: 'Revoke',
      danger: true,
      onConfirm: async () => {
        store.confirm.revoke = null;
        try {
      ...
      onCancel: () => { store.confirm.revoke = null; render(); },
    });
  }
...
function renderCredentials() {
  ...
  renderSudoRemoveControl(a.credentialPresent);
...
function renderSudoRemoveControl(present) {
  const slot = $('sudo-remove-slot');
  clear(slot);
  if (store.confirm.sudoRemove) {
    slot.appendChild(confirmStrip({
      message: COPY.credRemoveConfirm(selfMachine()),
      confirmLabel: 'Remove',
      danger: true,
      onConfirm: async () => {
        store.confirm.sudoRemove = false;
        try {
      ...
      onCancel: () => { store.confirm.sudoRemove = false; render(); },
    }));
    return;
  }
  const button = el('button', 'danger', 'Remove the stored password');
  button.type = 'button';
  button.id = 'sudo-remove';
  button.disabled = !present;
  button.addEventListener('click', () => { store.confirm.sudoRemove = true; render(); });
...
// A generic restart control shared by the fleet card and the setup identity card: both trigger
// the same POST /api/ui/restart behind the same inline confirm, they just render into different
// containers and carry their own confirm-state key so opening one does not open the other.
function renderRestartControl(container, resultId, confirmKey, buttonClass) {
  if (!container) return;
  clear(container);
  if (store.confirm[confirmKey]) {
    container.appendChild(confirmStrip({
      message: COPY.restartConfirm(selfMachine()),
      confirmLabel: 'Restart',
      danger: true,
      onConfirm: async () => {
        store.confirm[confirmKey] = false;
        try {
      ...
      onCancel: () => { store.confirm[confirmKey] = false; render(); },
    }));
    return;
  }
  const button = el('button', buttonClass || 'danger', 'Restart the daemon');
  button.type = 'button';
  button.addEventListener('click', () => { store.confirm[confirmKey] = true; render(); });
...
function renderParsed() {
  ...
  if (parsed.error) {
    node.appendChild(el('span', 'bad', `${REFUSAL_ADVICE['bad-argv']()} (${parsed.error})`));
    return;
  }
...
function wireAdmin() {
  ...
  $('run-form').addEventListener('submit', async (ev) => {
    ...
    if (parsed.error || !parsed.argv || !parsed.argv.length) {
      result('run-result', parsed.error ? `${REFUSAL_ADVICE['bad-argv']()} (${parsed.error})` : COPY.runNeedsCommand, 'bad');
      return;
    }
    if (!reason) { result('run-result', COPY.runNeedsReason, 'bad'); $('run-reason').focus(); return; }
    // Client-side friction only: the target daemon is what actually enforces refusal rules.
...
function refusalText(refusal, machine) {
  const fn = REFUSAL_ADVICE[refusal];
...
async function pollRun() {
  ...
  try {
    ...
    // Backs off x4 per transient failure, same as the web console.
    runPollFailures += 1;
